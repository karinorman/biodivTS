...
change_group_ses_desc <- change_group_count_ses %>%
   mutate(change_description = case_when(
    S == "pos" ~ "increase in species richness, sometimes leading to increase in functional richness", 
    S == "neg" ~ "loss of redundancy", 
    S == "0" ~ "functional change independent of species richness change",
    n == 7 ~ "no change", 
    .default = NA
  ))
```
...
```{r}
no_change <- change_groups %>% 
  mutate(study_id = as.integer(study_id)) %>%
  filter(across(!study_id, ~. == 0)) %>%
