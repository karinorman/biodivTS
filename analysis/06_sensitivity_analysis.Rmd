...
```{r}
dur_dfs <- list(min3 = filter(model_data, taxa != "Amphibians", duration > 2),
                min4 = filter(model_data, taxa != "Amphibians", duration > 3),
                min5 = filter(model_data, taxa != "Amphibians", duration > 4))
...
```{r}
with_amph <- pmap_dfr(filter(mods, model_id == "base"), get_model, data = model_data) %>%
  mutate(dataID = "with_amph")
```
...
sens_mods <- bind_rows(duration_sens, coverage_sens, with_amph) %>%
  select(-model) %>%
  filter(metric != "Jaccard_next") %>%
  write_csv(here::here("paper", "sensitivity_models.csv"))
```
