...
plot_slopes <- function(metric_name, plot_label, keep_x_axis, color){
  ...
  metric_plot <- plot_data %>% 
    ...
    annotate('text', x = -2.5, y = 0.8, label = plot_label, size = unit(6, "pt")) 
  ...
  if(gen_est$p.adjust < 0.05){
    metric_plot <- metric_plot + annotate('text', x = (gen_est$estimate_scale + .4), y = 0.8, label = "*", size = unit(16, "pt"))
  }
  
  return(metric_plot)
}  

keep <- pmap(slope_plot_df %>% filter(metric_name == "CWM_diet_planto"), plot_slopes)
keep 
...
cowplot::save_plot(here::here("figures/slope_plot_grid.png"), slope_plot_grid, ncol = 7, nrow = 5, bg = "white", base_asp = .9)
