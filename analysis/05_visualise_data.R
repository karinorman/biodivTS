...
cell_count <- rarefied_medians %>%
  group_by(biome_map, taxa_mod) %>%
  dplyr::summarise(n_cells = n_distinct(rarefyID)) %>%
  ungroup()

##	rejoin
rarefied_medians <- left_join(cell_count, rarefied_medians, by=c('biome_map', 'taxa_mod'))
...
newdat <-  rarefied_medians %>%
               distinct(biome_map, taxa_mod, STUDY_ID, rarefyID,
                        SamplePool, SampleN, num_years,
...
newdat %>%
  ...
  group_by(REALM, biome_map, taxa_mod) %>%
