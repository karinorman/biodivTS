...
biotime_traits <- comp_biotime_traits %>%
  ...
  filter(study_id %in% inc_coverage$study_id, !is.na(scientificName), !is.na(plot)) %>% 
...
Create one weight column 
```{r}
biotime_traits <- biotime_traits %>%
  mutate(weight = case_when(
    year_abun != 0 ~ year_abun,
    TRUE ~ year_biomass
  )) %>%
  select(-c(year_abun, year_biomass))
```
...
study_table <- biotime_data %>%
  group_by(study_id) %>%
  mutate(min_year = min(year), max_year = max(year), n_year = n_distinct(year), plots = n_distinct(plot)) %>% 
  select(study_id, taxa, min_year, max_year, plots) %>%
  distinct() %>%
