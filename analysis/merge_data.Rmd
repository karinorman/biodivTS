...
biotime_traits <- comp_biotime_traits %>%
  filter(study_id %in% inc_coverage$study_id, !is.na(scientificName)) #get studies with appropriate coverage and drop observations that we don't have trait data for
```
...
First pass at calculating FD metrics
```{r}
data <- biotime_traits %>% 
...
plan(multiprocess)
system.time(fd <- future_pmap_dfr(combs, ~ get_fd(data = data, study = .x, plot = .y)))
```
...
```{r}
model_data <- fd_table %>% filter(metric == "FRic", study_id == 311)
fit <- glm(value ~ year + richness, data = model_data)
