...
check if all bird & mammal biotime data w/id's has an elton match. There are 73 species without a match (63 are birds).
```{r}
...
and 20136 species without an ID, some of which are species codes
```{r}
biotime_data %>%
  #filter(taxa == "Birds") %>%
  select(id, sourceName, taxa) %>%
