...
get_fd <- function(data, study, plot_num){
  ...
  as_tibble(dbFD_joggle(trait_mat, species_mat, w.abun = FALSE))[,1:8] %>%
    cbind(years, .) %>%
    mutate(study_id = unlist(study), plot = unlist(plot_num)) %>%
...
fd <- map_dfr(combs, ~ get_fd_safe(data = data, study = .x[1], plot = .x[2]))  
```
