...
get_fd <- function(obs_data, filter_plot, ...){
  ...
  species_mat <- data %>%
    select(year, id, year_abun) %>%
    arrange(year) %>%
    arrange(id) %>%
    pivot_wider(names_from = id, values_from = year_abun, values_fill = 0) %>%
    select(-year)
  ...
  trait_mat <- data %>%
    select(-c(biotimeName, taxa, id_species, plot, year, year_abun, 
              year_biomass, traitName, scientificName, plot_id, study_id)
    ) %>%
    distinct() %>%
  ...
  bin_vars <- names(bin_vars[bin_vars == TRUE])
    
  trait_mat <- trait_mat %>%
    mutate_at(vars(-c(id, bin_vars), -ends_with("5cat")), 
              list(~as.numeric(scale(.)))) %>% #rescale variables, not binary or categorical
    mutate_at(vars(bin_vars), as.character(.)) %>%
    arrange(id) %>%
  ...
  fd_out <- as_tibble(dbFD_joggle(trait_mat, species_mat, w.abun = TRUE, m = "min", ...))[,1:8] %>%
    cbind(years, .) %>%
...
#system.time(fd <- map(plot_sizes$plot_id, get_fd_error, obs_data = data, corr = "cailliez"))
system.time(fd <- map(plot_sizes$plot_id, get_fd_safe, obs_data = data, corr = "cailliez"))
```
