...
get_parallel_FD <- function(data, group_num){
  ...
  future::plan(multiprocess)
  fd <- future_map(group_plots$plot_id, get_fd_safe, data = group_data) %>%
    rbind()
...
fd <- map(1:num_groups, get_parallel_FD, data = data)
```
...
plots_left <- plots[!plots %in% fd_data$plot_id]

library(furrr)
future::plan(multiprocess)
system.time(fd <- future_map(plots_left, get_fd_safe, data = data))
```
