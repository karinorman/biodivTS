...
get_nullplot_FD <- function(file_path, traits, ...) {
  ...
  plot_samples <- read_csv(file_path) %>% mutate(plot_id = as.character(plot_id)) %>% replace(is.na(.), 0)
  ...
  setwd(here::here())
  write_tsv(sample_fd, paste0(save_dir, "/null", unique(plot_samples$plot_id), ".csv"))
...
null_files <-  dir(path, "*.csv") %>%
  paste0(path, "/", .)

process_nulls <- function(file_path){
...
plan(multiprocess)
null_table <- future_map_dfr(null_files, process_nulls)
