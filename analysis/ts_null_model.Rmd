...
get_plot_sample <- function(data, filter_plot, n){
  ...
  get_samp_species <- function(data){
    ...
    samp_abun <- sample(data$year_abun)
...
# sample_occurrence <- map(unique(sample_plots$plot_id), get_plot_sample, data = sample_plots, n = n) %>%
#   bind_rows() %>%
#   replace(is.na(.), 0)
# pins::pin(sample_occurrence, board = "github")
sample_occurrence <- pins::pin_get("sample_occurrence", board = "github")

get_plot_FD <- function(data, occ_df, filter_plot, ...) {
  plot_samples <- occ_df %>%
...
null_FD <- map_dfr(unique(sample_plots$plot_id)[1:3], get_plot_FD, 
                   data = sample_plots, occ_df = sample_occurrence,
                   m = "min", corr = "cailliez")
```
...
get_parallel_FD <- function(data, group_num){
  ...
  fd <- map_dfr(group_plots$plot_id[1:3], get_plot_FD, 
                   data = group_data, trait_range = 18:65, occ_df = sample_occurrence)
