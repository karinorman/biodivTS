...
feve_fit <- glmer(value ~ year + (1|study_id/rarefyID), data = eve, family = Gamma,
                  control = glmerControl(optCtrl = list(maxit = 1e9,
                                                    maxfun = 1e9,
                                                    xtol_abs = 1e-11,
                                                    ftol_abs = 1e-11,
                                                    maxeval = 1e9)))
...
#find best optimizer
diff_optims <- allFit(feve_fit, maxfun = 1e5)


optimx_options <- c("L-BFGS-B", "nlminb", "nlm", "bobyqa", "nmkb", "hjkb")

for(i in 1:length(optimx_options)){
  model_flex <- glmer(value ~ year + (1|study_id/rarefyID), data = eve, family = Gamma,
                     control = glmerControl(optimizer = "optimx",
                                           optCtrl = list(method = optimx_options[i],
                                                                   maxit = 1e9)))
  print(model_flex@optinfo$conv$lme4$messages)
}
