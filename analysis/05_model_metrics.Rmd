...
SES_rand_slope <- model_data %>%
  ...
  group_nest(metric) %>%
  mutate(#year_center = map(data, ~scale(.x$year)),
         model = map(data, ~lmer(value ~ taxa + (1|study_id) + (|study_id:rarefyID), 
                                 data = .x)),
           # null_model = map(data, ~lmer(value ~ (year_scaled|study_id/rarefyID), 
           #                       data = .x)),
           # test = map2(model, null_model, anova),
...
taxa_slope <- study_slope_data %>%
  ...
  mutate(#year_center = map(data, ~scale(.x$year)),
         ...
         model = map(data, ~glm(estimate ~ taxa, 
                                 data = .x)),
           coef = map(model, tidy),
           ) %>% 
  select(metric, coef) %>%
  unnest(cols = c(coef)) %>%
  mutate(covariate = "taxa")
