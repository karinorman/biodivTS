...
metric_df <- bind_rows(tibble(metric_name = raw_metrics, 
                         formula = "value ~ year_scaled + (year_scaled|rarefyID)"),
          tibble(metric_name = logone_metrics, 
                         formula = "logvalue ~ year_scaled + (year_scaled|rarefyID)"),
          tibble(metric_name = log_metrics, 
                         formula = "log(value) ~ year_scaled + (year_scaled|rarefyID)")) %>%
...
mixed_indv_mod_results <- pmap_dfr(mixed_indv_model_df %>% filter(!metric_name %in% c("Jaccard_base", "Jaccard_next")), get_model, data = model_data)
...
##FIXME: some of these still don't converge##
```
...
bootstrap_slopes <- pmap_dfr(model_df %>% filter(model_id == "base", metric == "CWM_bodymass_value") %>% mutate(nsim = 100), bootstrap_study_slopes) %>%
