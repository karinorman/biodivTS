...
get_model <- function(metric_name, formula, model_id, data, ...){
  ...
  tidy(fit$result) %>%
   mutate(metric = metric_name, model = list(fit), model_id = model_id, warning = ifelse(length(fit$warnings) == 0, NA, fit$warnings))
}
...
study_slope_data <- study_slopes %>%
  ...
  left_join(model_data %>% select(study_id, startyear, duration) %>% distinct())

taxa_slope <- study_slope_data %>%
  mutate(taxa = as.factor(taxa)) %>%
  group_nest(metric) %>%
  mutate(#year_center = map(data, ~scale(.x$year)),
         model = map(data, ~lmer(estimate ~ (1|study_id) + year_scaled + (year_scaled|study_id/rarefyID), 
                                 data = .x)),
           # null_model = map(data, ~lmer(value ~ (year_scaled|study_id/rarefyID), 
           #                       data = .x)),
           # test = map2(model, null_model, anova),
...
taxa_slope <- study_slope_data %>%
  ...
  #filter(metric %in%  c("SES_FRic", "SES_FEve", "SES_FDiv", "Jaccard_next")) %>%
  group_nest(metric) %>%
  mutate(model = map(data, ~glm(estimate ~ taxa, 
                                 data = .x)),
           tukey_mod = map(model, ~glht(., mcp(taxa="Tukey"))),
         tukey_coef = map(tukey_mod, tidy)) %>% 
  select(metric, tukey_coef) %>%
  unnest(cols = c(tukey_coef)) %>%
  rename(covariate = term)

climate_slope <- study_slope_data %>%
  #filter(metric %in%  c("SES_FRic", "SES_FEve", "SES_FDiv", "Jaccard_next")) %>%
  group_nest(metric) %>%
  mutate(#year_center = map(data, ~scale(.x$year)),
         #model = map(data, ~lmer(estimate ~ taxa + (1|taxa), 
         model = map(data, ~glm(estimate ~ climate, 
                                 data = .x)),
           tukey_mod = map(model, ~glht(., mcp(climate="Tukey"))),
         tukey_coef = map(tukey_mod, tidy)) %>% 
  select(metric, tukey_coef) %>%
  unnest(cols = c(tukey_coef)) %>%
  rename(covariate = term)

realm_slope <- study_slope_data %>%
  #filter(metric %in%  c("SES_FRic", "SES_FEve", "SES_FDiv", "Jaccard_next")) %>%
  group_nest(metric) %>%
  mutate(#year_center = map(data, ~scale(.x$year)),
         #model = map(data, ~lmer(estimate ~ taxa + (1|taxa), 
         model = map(data, ~glm(estimate ~ realm, 
                                 data = .x)),
           tukey_mod = map(model, ~glht(., mcp(realm="Tukey"))),
         tukey_coef = map(tukey_mod, tidy)) %>% 
  select(metric, tukey_coef) %>%
  unnest(cols = c(tukey_coef)) %>%
  rename(covariate = term)
...
duration_slope <- study_slope_data %>%
  ...
  mutate(#year_center = map(data, ~scale(.x$year)),
         #model = map(data, ~lmer(estimate ~ taxa + (1|taxa), 
         model = map(data, ~glm(estimate ~ duration, 
                                 data = .x)),
           coef = map(model, tidy),
           ) %>% 
  select(metric, coef) %>%
...
startyear_slope <- study_slope_data %>%
  ...
  mutate(#year_center = map(data, ~scale(.x$year)),
         #model = map(data, ~lmer(estimate ~ taxa + (1|taxa), 
         model = map(data, ~glm(estimate ~ startyear, 
                                 data = .x)),
           coef = map(model, tidy),
           ) %>% 
  select(metric, coef) %>%
...
slope_models <- bind_rows(taxa_slope, realm_slope, climate_slope, duration_slope, startyear_slope)
