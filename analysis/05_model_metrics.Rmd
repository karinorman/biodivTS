...
```{r}
# raw_value_mods <- tibble(metric_name = c("SES_FRic", "SES_FEve", "SES_FDiv", "FRic", "FEve", "FDiv", "Jaccard_base"), 
...
raw_value_mods <- tibble(metric_name = c("SES_FRic", "SES_FEve", "SES_FDiv", "FRic", "FEve", "FDiv", "Jaccard_base"), 
                         base = "value ~ year_scaled + (year_scaled|study_id/rarefyID)",
                         taxa = "value ~ year_scaled + as.factor(taxa) + (year_scaled|study_id/rarefyID)",
                         realm = "value ~ year_scaled + as.factor(realm) + (year_scaled|study_id/rarefyID)",
                         climate = "value ~ year_scaled + as.factor(climate) + (year_scaled|study_id/rarefyID)") %>%
  pivot_longer(cols = c(base, taxa, realm, climate), names_to = "model_id", values_to = "formula")

log_value_mods <- tibble(metric_name = c("S"), 
                         base = "logvalue ~ year_scaled + (year_scaled|study_id/rarefyID)",
                         taxa = "logvalue ~ year_scaled + as.factor(taxa) + (year_scaled|study_id/rarefyID)",
                         realm = "logvalue ~ year_scaled + as.factor(realm) + (year_scaled|study_id/rarefyID)",
                         climate = "logvalue ~ year_scaled + as.factor(climate) + (year_scaled|study_id/rarefyID)") %>%
  pivot_longer(cols = c(base, taxa, realm, climate), names_to = "model_id", values_to = "formula")
...
metric_model_table %>% 
  ...
  select(-c(model, model_id, warning, formula, statistic, df)) %>% 
...
int_raw_value_mods <- tibble(metric_name = c("SES_FRic", "SES_FEve", "SES_FDiv", "FRic", "FEve", "FDiv", "Jaccard_base"), 
                         ...
                         taxa_dur = "value ~ year_scaled + duration_scaled*year_scaled + (year_scaled|study_id/rarefyID) + (year_scaled|taxa)",
                         realm_dur = "value ~ year_scaled + duration_scaled*year_scaled + (year_scaled|study_id/rarefyID) + (year_scaled|realm)",
                         climate_dur = "value ~ year_scaled + duration_scaled*year_scaled + (year_scaled|study_id/rarefyID) + (year_scaled|climate)",
                         base_srt = "value ~ year_scaled + startyear_scaled*year_scaled + (year_scaled|study_id/rarefyID)",
                         taxa_srt = "value ~ year_scaled + startyear_scaled*year_scaled + (year_scaled|study_id/rarefyID) + (year_scaled|taxa)",
                         realm_srt = "value ~ year_scaled + startyear_scaled*year_scaled + (year_scaled|study_id/rarefyID) + (year_scaled|realm)",
                         climate_srt = "value ~ year_scaled + startyear_scaled*year_scaled + (year_scaled|study_id/rarefyID) + (year_scaled|climate)") %>%
...
#Fit models with base settings
int_mod_results <- pmap_dfr(int_raw_value_mods, get_model, data = model_data)
