...
tibble(Trait = c(rep("Diet", 10), rep("Foraging Strata", 7), rep("Pelagic Specialist", 2), rep("Nocturnal", 2), "Body Mass"),
       Category = c("Invertebrate", "Mammals and Birds", "Reptiles", "Fish", "Unknown Vertebrates", "Scavenging", "Fruit", "Nectar", "Seeds", "Other Plant", 
                    ...
                    "yes", "no",
                    "-"),
       `Data Type` = c(rep("percentage consumed", 10), rep("percentage of use", 7), rep("binary", 4), "continous, in grams")) %>%
...
`r colorize("In general, functional capacity of mammals shows greater declines that bird communities.", "blue")`

`r colorize("We also found evidence of increasing evenness for Marine studies. The empirical link between changes in evenness and ecosystem process is the most poorly studied of the metrics, however theory indicates that increased evenness improves function and stability ", "blue")` [@filstrup2019], `r colorize("as the dominance of one or a few species makes function sensitive to the ability of those species to respond to environmental change ", "blue")`[@hillebrand2008; @hillebrand2009].
...
`r colorize("The addition of functional trait data illuminates a few key areas where a synthesis approach is incongruous with system-specific studies or expectations based on anthropegenic impacts. For example, we found no evidence of general trends in the *CWM* for body mass or for studies aggregated by their characteristics despite predictions that mean body mass is decreasing as a result of climate change impacts and megafaunal loss ", "blue")`[@sheridan2011] `r colorize(", and downgrading documented empirically and experimentally in multiple taxa ", "blue")`[@caruso2014; @forster2012; @huss2019; @tseng2018].
`r colorize("Still, climate change impacts are predicted to be most extreme for ectotherms and impact intraspecific traits first before species are lost, neither of which are represented by our data. Further, many of the studies in our dataset draw from areas that may have experienced significant loss of large-bodied species before the observation window, with contemporary loss rates slowing", "blue")` [@fritz2009].
...
`r colorize("Results for protected areas were also surprising given the assumption that protection insulates communities from functional degradation. The only difference we found between protected and unprotected areas was significant turnover occurring outside protected areas and no significant turnover inside protected areas. Rather than the result of protection itself, this is likely due to the fact that the majority of studies were form terrestrial, temperate studies where turnover is known to be lower ", "blue")`[@blowes2019].
