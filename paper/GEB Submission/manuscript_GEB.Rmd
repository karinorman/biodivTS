...
`r colorize("Surprisingly, we found no evidence for some of the strongest predictions for anthropogenic impact. For example, we found no trend for mean body mass in bird or mammal communities. This result could be an indication that either body size is not changing significantly due to climate change, opposing pressures such as urbanization are overshadowing climate change impacts, or more likely current shifts are happening at an intraspecific level not captured by our data. Additionally, many of the studies in our dataset draw from areas that may have experienced significant loss of large-bodied species before the observation window, with contemporary loss rates slowing", "blue")` [@fritz2009].
...
change_group_ses_desc %>%
  ...
  rename(`Study Count` = `n`) %>%
     kable(format = "latex",
        caption = "Table giving the count of number of studies for different kinds of functional change, broken down into no change functional change without species richness change, loss of redundancy, and increases in species richness.", align = "c",
...
`r colorize("The final two studies showed an increase in species richness and uncorrected functional richness, with only one showing an increase in functional richness beyond what was expected by chance (corrected functional richness). These communities have a hopeful trajectory, as they are improving their functional capacity and potentially their functional redundancy, likely leading to robust future ecological function. Notably, both of these studies were from temperate, terrestrial bird communities in Scandinavian countries, which have some of the highest investments in conservation in the world. Still, results are consistent with other broad-scale studies of bird communities, which found loss of common, functionally general species alongside increases in rare species ", "blue")` [@inger2015; @rosenberg2019; @schipper2016].
