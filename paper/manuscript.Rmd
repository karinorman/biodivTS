...
We also calculated the community-weighted mean (*CWM*) of all included traits to examine shifts in individual trait distributions.
...
For clarity in assessing the large number of *CWM*'s, we report only these general trends for each trait *CWM*.
All timeseries with data for a given trait were included in the model.
...
<!-- something here about models be segregated by taxa already, since most traits weren't for all taxa -->
...
We found no significant overall trend in species richness or summary functional diversity metrics (observed or standardized) (Fig 2).
...
However, with only two global studies and two freshwater studies these results are likely a characterization of the limited data rather than overall trends.
...
read_csv(here::here("figures/man_model_table.csv")) %>%
  ...
  arrange(metric) %>%
  kable(format = "latex", caption = "Model estimates and statistics for general trend models for species richness (S) and standardized summary metrics. Additional model estimates, including CWM models, can be found in the supplement.") %>%
...
Additionally, results from *CWM* models show that there were very few general shifts in functional trait values across communities.
...
For example, mean body size is predicted to decrease as a result of climate change impacts and megafaunal loss [@sheridan2011], a phenomena which has already been well documented empirically and expirimentally in multiple taxa [@caruso2014; @forster2012; @huss2019; @tseng2018].
We found no evidence of a trend in the community-weighted mean of body size.
Still, species-level trait means can only capture distribution shifts due to complete species losses.
Intraspecific changes may still be occurring in these communities, particularly if species are being impacted by environmental changes but have not yet been fully extirpated from a community.
...
Still, for many timeseries we likely did not have a large enough time window to capture community and species pool impoverishment due to extinction.
...
These findings do not negate a substantial body of work linking functional degradation to direct human intervention in the form of land use change and intensification or habitat fragmentation [@flynn2009; @magioli2021; @tinoco2018], but rather illustrate the prevailing functional state for communities experiencing background levels of environmental change.
...
Code for the analyses in this chapter is archived on Zenodo at \<[[DOI:10.5281/zenodo.5514335\\\\](DOI:10.5281/zenodo.5514335\){.uri}]([DOI:10.5281/zenodo.5514335\\](DOI:10.5281/zenodo.5514335)%7B.uri%7D){.uri}\> [\@norman_karinormanbiodivts_2021]. Data products are on Github at <https://github.com/karinorman/biodivTS_data>. Original data sources are open access and available at their respective providers.
...
Support for this work was provided by United States Department of Energy through the Computational Sciences Graduate Fellowship (DOE CSGF) under grant number DE-FG02-97ER25308 awarded to K.E.A.N..
