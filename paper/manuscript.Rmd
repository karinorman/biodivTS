...
invasive species. Impacts of these pressures are well documented on a
global scale with an accelerating global extinction rate (citation) and
...
time have shown a decoupling of community composition from richness
[@brose2016], evidence of community-level regulation of richness
[@gotelli2017], and no net change in local biodiversity despite
widespread environmental change and ongoing species turnover
[@brown2001; @dornelas2014; @vellend2013; , @vellend2017]. While
...
metrics are particularly suited for understanding local-scale
biodiversity patterns as they can detect shifts in community structure
independent of species change.

<!-- Functional diversity also sheds light on critical ecological processes. -->

Functional diversity also sheds further light on critical ecosystem and
community processes. For example, there is increasing evidence that
functional diversity is a better predictor of ecosystem function than
species-based metrics [@cadotte2011; @gagic2015], and that different
facets of functional diversity also play a role in maintaining ecosystem
stability [\@craven2018; @morin2014]. Further, biogeographic assessments
of functional diversity give foundational insight into how community
...
broad-scale [@trindade-santos2020; @jarzyna2016; @barnagaud2017].
<!-- so what do these studies say? does that go here? --> However to
date there has been no comprehensive assessment of functional change
through time on a broad geographic and taxonomic extent.\

Here we perform the first cross-taxa and cross-system assessment of
functional diversity change through time. We assess 100's of mammal,
bird, and amphibian functional diversity timeseries to determine how
functional diversity change compares to species change. We also identify
3 distinct types of functional change common across communities and
their implications for ecological processes.
...
database, a global repository of high quality assemblage timeseries. All
studies included in the database follow consistent sampling protocols
and represent full assemblages rather than population-level data
[@dornelas2018]. Following best practices for the database [@blowes2019]
and to maintain a consistently defined local scale, studies with
multiple sample locations were split into separate time series based a
global grid wherein all samples within a single grid cell were
considered a single time series. The size of the cell was determined
based on the sample extent of the single location studies (see
...
for this sampling effort inconsistency we used sample-based rarefaction
by bootstrap resampling within years for each timeseries based on the
smallest number of samples in a year for that timeseries.
...
based on the methods for each study.

#### *Analysis*

We modeled each diversity metric using a Generalized Linear Mixed
Effects model with year as the predictor and mixed effects of timeseries
nested within study. <!-- more to come! -->
