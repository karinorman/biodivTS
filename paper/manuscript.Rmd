...
We assess thousands of mammal, bird, and amphibian functional diversity timeseries to determine f there is a general trend of functional change.
...
where are the increases/decreases occurring?

any differences in some of the taxonomic groups even if not significant?

<!-- how does this reconcile with the large body of studies showing that extinctions primarily affect functionally rare species? It seems inconsistent with their findings --> Maintenance of functional richness also goes against predictions given by work linking extinction probability to functional uniqueness.
A large body of work demonstrates that in x,y,z communities functionally unique species are more likely to go extinct.
At the scale of a local community, we would expect these species would also be first to be locally extirpated as population sizes decrease and ranges shrink and become more sparse.
...
We join others in the call for increased monitoring in under sampled areas and continued efforts to centralize existing data sources [@gonzalez2016; @hughes2021; @vellend2017]. Data that fill geographic, taxonomic and trait gaps should be prioritized over further collection of data that replicate existing biases. One relatively low-cost high-reward data investment is collation of additional species-level trait means. Intentional trait selection is critical for linking functional patterns to ecological processes [@zhu2017], however synthesis is constrained to the traits in a few taxa-specific databases. Traits would be especially beneficial if they explicitly fit into existing frameworks for linking traits to processes, including the now widely accepted response and effect framework [@lavorel2002].
