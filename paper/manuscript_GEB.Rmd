...
Global analyses have been heavily criticized for geographic biases, lack of data in the most heavily impacted areas, and for not taking into account the ecological context of individual studies [@cardinale2014; @cardinale2018; @gonzalez2016].
...
Still, some losses of functional diversity are indisputable on both paleocological and contemporary timescales such as continued trophic downgrading due to loss of large-bodied mammals, but implications of those losses for local diversity patterns are less clear [@estes2011; @smith2018] .
...
Further, with only two global studies, the trend should not be considered truely general.
...
Critically, while trends for functional metrics may be statistically significant, the effect size necessary to be ecologically significant for key processes remains an outstanding question of the field.
...
While the majority of studies in our dataset did not experience a significant functional loss, a substantial body of work links functional degradation to species losses as a result of direct human intervention in the form of land use change and intensification or habitat fragmentation [@flynn2009; @magioli2021; @tinoco2018].

## Future Work

We present here four prevailing scenarios of change experienced in bird and mammal communities.
Incorporation of additional data to fill geographic and taxanomic data gaps would be able to give us a clear picture which of these scenarios is most prevalent.
However, data takes time to collect and new sources can be costly to incproporate.
In the meantime then, we can make significant strides in linking the scenarios of change to common drivers.
Do different drivers result in different signatures of change?

While the trends we see here are statistically significant, we still do not know if they are ecologically significant.
There is substantial work to be done to link degrees of change in functional structure to measurable outcomes in ecosystem processes.
...
We present the conclusion best supported by available data and acknowledge that overall trends would likely look different were underlying data a true global sample.
