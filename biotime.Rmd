...
col_ex <- c("specid", "passnonpass", "iocorder", "blfamilylatin", "blfamilyenglish", "blfamsequid", "taxo")

bird_trait_data <- bird_ids  %>%
  select(id = acceptedNameUsageID, input) %>%
  distinct() %>%
  right_join(bird_trait %>% select(.dots = -col_ex), by = c("input" = "scientific")) %>%
  rename(originalSciName = input)
```
