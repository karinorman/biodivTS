...
syn_res <- bind_rows(by_name(na.omit(unique(alt_names$synonym)),"itis"),
                     by_name(na.omit(unique(alt_names$acceptedNameUsage)),"itis")) %>%
  select(id = acceptedNameUsageID, input, scientificName) %>% 
  drop_na(id) %>% 
  distinct() %>%
  inner_join(data %>% select(-species, -genus), by = c("input" = "genus_species"), na_matches =  "never") %>%
  rename(matchingName = input) 
