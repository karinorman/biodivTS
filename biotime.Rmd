...
syn_res <- bind_rows(synonyms(biotime_data %>% filter(is.na(id)) %>% pull(matchingName) %>% unique(), "ott"),
                  synonyms(biotime_data %>% filter(is.na(id)) %>% pull(matchingName) %>% unique(), "gbif"),
                  synonyms(biotime_data %>% filter(is.na(id)) %>% pull(matchingName) %>% unique(), "col"),
                  synonyms(biotime_data %>% filter(is.na(id)) %>% pull(matchingName) %>% unique(), "ncbi"),
                  synonyms(biotime_data %>% filter(is.na(id)) %>% pull(matchingName) %>% unique(), "wd"),
                  synonyms(biotime_data %>% filter(is.na(id)) %>% pull(matchingName) %>% unique(), "iucn")
                  ) %>%
  pull(synonym) %>% 
  na.omit() %>% 
  unique() %>%
  ...
  select(id = acceptedNameUsageID, input, scientificName) %>% 
  drop_na(id) %>% 
  left_join(data %>% select(-species, -genus), by = c("input" = "genus_species")) %>%
  rename(matchingName = input)
...
#These two should be equal, why aren't they??
dim(syn_res)  
biotime_data %>% filter(matchingName %in% syn_res$matchingName) %>% dim()
biotime_data %>% filter(is.na(id)) %>% dim()
```
